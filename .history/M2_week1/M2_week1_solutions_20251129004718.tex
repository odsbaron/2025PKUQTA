...
计算积分项得 $\frac{L^3}{6}$ 与 $\frac{2kL^3}{3}$，代入化简得 $k = \frac{1}{6} + \frac{2}{3}k$，解得 $k = \frac{1}{2}$。因此当 $L=1$ 时，极限值为 $E(1) = \frac{1}{2}$。
    ...
    为了构建一个能够 100\% 覆盖所有可能密码的确定性方案，我们采用基于模运算的“黄金 60”构造法。该构造将总尝试集合定义为 6 个子集 $S_k$ 的并集，总计 60 次尝试。对于每个子集，通过遍历首位数字 $x \in \{0, \dots, 9\}$，并依据同余方程：
    \begin{align}
        y &\equiv (x + 2k) \pmod{10} \\
        z &\equiv (x + k) \phantom{2} \pmod{10}
    \end{align}
    确定后续两位，生成 10 个互不干涉的空间对角线码字。核心在于参数 $k$ 的选取——我们严格选定：
    \[
    k \in \{0, 1, 2, 3, 5, 7\}
    \]
    这组特定的参数巧妙利用了三维空间中 $xy$、$xz$ 及 $yz$ 平面投影差值的互补性，确保了无论目标密码的三位数字间距为何，都能落入覆盖网中。
    
    \section*{覆盖原理简述}
    
    其覆盖的完备性源于差值集合的无缝拼接：
    \begin{itemize}
        \item $y-x$ 的差值（即 $2k \pmod{10}$）覆盖了偶数集合 $\{0, 2, 4, 6\}$；
        \item $z-x$ 的差值（即 $k \pmod{10}$）覆盖了奇数集合 $\{1, 3, 5, 7\}$；
        \item $z-y$ 的差值（即 $-k \pmod{10}$）补足了高位差值 $\{8, 9\}$。
    \end{itemize}
    三者之并集恰好为 $\{0, \dots, 9\}$ 的全集，从而在数学上证明了覆盖的必然性
