...
\begin{enumerate}[label=\arabic*., itemsep=2.5em]
    ...
    \textbf{解法：}递推关系。设 $S(L)$ 为长度 $L$ 的线段经过无限次切割后的期望和。长度为 $L$ 的线段随机切开，两段乘积的期望为 $\frac{L^2}{6}$。设 $S(L) = cL^2$，则：
    \[
    S(L) = \frac{L^2}{6} + 2c \cdot \frac{L^2}{3}
    \]
    ...
    \textbf{解法：}De Bruijn序列。需要构造包含所有 $10^2 = 100$ 种两位数组合的最短序列。图论建模：10个数字为顶点，从 $i$ 到 $j$ 的有向边代表两位数 $ij$，共100条边。问题等价于找欧拉回路（每条边恰好经过一次），长度 = 边数 = 100。
