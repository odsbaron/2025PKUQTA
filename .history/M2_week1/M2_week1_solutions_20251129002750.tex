...
计算积分项得 $\frac{L^3}{6}$ 与 $\frac{2kL^3}{3}$，代入化简得 $k = \frac{1}{6} + \frac{2}{3}k$，解得 $k = \frac{1}{2}$。因此当 $L=1$ 时，极限值为 $E(1) = \frac{1}{2}$。
    ...
    然而，这仅是无法突破的绝对物理底线，因为在 $10 \times 10 \times 10$ 的离散网格中，固定的“十字形”覆盖域无法在不发生重叠的情况下完美拼合。所以需要可以找到一些实际可以构造的下界。
    
    我们将尝试集合 $C$ 定义为 $5$ 个子集 $S_k$ 的并集，总尝试次数为 $N = 50$。该构造利用模运算在三维空间中生成了 5 条互不干涉的“空间对角线”，相比于固定某一维度的平面切片法（需 100 次），这种立体构造能更均匀地分布在解空间中。
...
\end{description}
\textbf{答案：}$\displaystyle 理论下界为36次，这里给了一个实际构造的下界50次$
    % 题目 4
    \item \textbf{把守圆圈}
