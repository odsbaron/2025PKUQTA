...
\begin{enumerate}[label=\arabic*., itemsep=2.5em]
    ...
    \textbf{答案：}$\displaystyle P = 1 - \frac{5}{84} = \frac{79}{84} \approx 94.05\%$
...
计算积分项得 $\frac{L^3}{6}$ 与 $\frac{2kL^3}{3}$，代入化简得 $k = \frac{1}{6} + \frac{2}{3}k$，解得 $k = \frac{1}{2}$。因此当 $L=1$ 时，极限值为 $E(1) = \frac{1}{2}$。
    ...
    首先，由于总状态空间为 $10^3$，而单次尝试作为一个汉明球，能覆盖自身及三个维度上各改变一位的 $9$ 个状态，单次总覆盖体积为 $1+9+9+9=28$；理论上若假设这些覆盖区域能如流体般完美填充空间，则所需最小次数为 $1000/28 \approx 35.71$，即至少 $36$ 次。然而，这仅是无法突破的绝对物理底线，因为在 $10 \times 10 \times 10$ 的离散网格中，固定的“十字形”覆盖域无法在不发生重叠的情况下完美拼合。正是这种离散几何结构带来的必然拼接损耗，使得实际构造无法达到理想化的 $36$ 次，目前已知最严谨的组合数学下界及最优构造解均维持在 \textbf{39 次}左右。
    \textbf{答案：}\textbf{100次}
    ...
    举例：\texttt{nums = [1, 2, 3, 4]}，$dp[2] = 1$（新增 [1,2,3]），$dp[3] = 2$（新增 [2,3,4] 和 [1,2,3,4]），总数 = $1 + 2 = 3$

    \textbf{答案：}$\sum_{i=2}^{n-1} dp[i]$，时间 $O(n)$，空间 $O(1)$
