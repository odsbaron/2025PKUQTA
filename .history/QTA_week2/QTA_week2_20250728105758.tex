...
因此，每次生成的比特都是公平的，即 $P(\text{输出0}) = P(\text{输出1}) =
\frac{1}{2}$。

\textbf{第二步：从公平比特生成0-15的均匀分布}

由于 $2^4 = 16 > 15+1 = 16$，我们需要4个公平比特来表示0-15的整数。

算法如下：
\begin{enumerate}
    \item 使用上述方法生成4个公平的随机比特 $b_3b_2b_1b_0$
    \item 将这4个比特转换为整数：$n = 8b_3 + 4b_2 + 2b_1 + b_0$
    \item 由于 $n \in \{0,1,2,\ldots,15\}$，直接输出 $n$
\end{enumerate}

\textbf{证明0-15均匀分布：}

由于每个比特都是独立且公平的（$P(b_i = 0) = P(b_i = 1) = \frac{1}{2}$），所以：
