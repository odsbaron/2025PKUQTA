...
\begin{enumerate}
    \item \textbf{建立模型}：将正方形的运动分解为当其中心沿着圆周移动一个无穷小的角度 $d\theta$ 时的过程。
    \item \textbf{计算微元面积 $dA$}：计算在这个无穷小角度 $d\theta$ 内，正方形所扫过的面积 $dA$。
    \item \textbf{积分求和}：将微元面积 $dA$ 从 $0$ 到 $2\pi$ (一整圈) 进行定积分，得出总面积 $A$。
\end{enumerate}
