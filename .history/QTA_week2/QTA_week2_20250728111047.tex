...
$$
\int_{-\infty}^{+\infty}(y^2 + \frac{2y}{\sqrt{\pi}} + \frac{1}{\pi})(1-\Phi(y))\varphi(y)dy}
$$
...
将积分展开可得到这个积分：
  ...
  展开 $(y+\frac{1}{\sqrt{\pi}})^2 = 

  原积分可分解为三部分：
  $$I = \int_{-\infty}^{+\infty} y^2(1-\Phi(y))\varphi(y)dy +
  \frac{2}{\sqrt{\pi}}\int_{-\infty}^{+\infty} y(1-\Phi(y))\varphi(y)dy +
  \frac{1}{\pi}\int_{-\infty}^{+\infty} (1-\Phi(y))\varphi(y)dy$$

  对于第三个积分：
  $$\int_{-\infty}^{+\infty} (1-\Phi(y))\varphi(y)dy = \int_{-\infty}^{+\infty}
  \varphi(y)dy - \int_{-\infty}^{+\infty} \Phi(y)\varphi(y)dy = 1 - \frac{1}{2} =
  \frac{1}{2}$$

  对于第二个积分，使用分部积分：
  $$\int_{-\infty}^{+\infty} y(1-\Phi(y))\varphi(y)dy = \frac{1}{\sqrt{2\pi}}$$

  对于第一个积分，利用递推关系：
  $$\int_{-\infty}^{+\infty} y^2(1-\Phi(y))\varphi(y)dy = \frac{1}{2} -
  \frac{1}{\sqrt{2\pi}}$$

  因此：
  $$I = \frac{1}{2} - \frac{1}{\sqrt{2\pi}} + \frac{2}{\sqrt{\pi}} \cdot
  \frac{1}{\sqrt{2\pi}} + \frac{1}{\pi} \cdot \frac{1}{2}$$

  $$= \frac{1}{2} - \frac{1}{\sqrt{2\pi}} + \frac{1}{\sqrt{\pi^2}} + \frac{1}{2\pi}$$

  $$= \frac{1}{2} - \frac{1}{\sqrt{2\pi}} + \frac{1}{\pi} + \frac{1}{2\pi} =
  \frac{1}{2} - \frac{1}{\sqrt{2\pi}} + \frac{3}{2\pi}$$
