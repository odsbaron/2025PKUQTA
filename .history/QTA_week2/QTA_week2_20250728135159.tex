...
小正方形C扫过的区域是一个圆环。该圆环的面积是其外边界构成的圆的面积减去其内边界构成的圆的面积。 
\begin{itemize}
    \item 我们假设正方形的几何中心沿着半径为5的圆周移动。
    \item \textbf{外圆半径 ($R_{\text{外}}$)}：由正方形最外侧的边扫过形成。其半径为大圆半径R加上半个正方形边长。
    $$ R_{\text{外}} = R + \frac{\text{边长}}{2} = 5 + \frac{1}{2} = 5.5 $$
    \item \textbf{内圆半径 ($R_{\text{内}}$)}：由正方形最内侧的边扫过形成。其半径为大圆半径R减去半个正方形边长。 
    $$ R_{\text{内}} = R - \frac{\text{边长}}{2} = 5 - \frac{1}{2} = 4.5 $$
\end{itemize}
