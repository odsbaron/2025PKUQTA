...
\section{核心思想：构造一个公平赌局}
为了应用鞅理论，我们构造一个如下的假想赌局：
\begin{enumerate}
...
\subsection{应用鞅停时定理}
我们定义一个**停时** $\tau$ 为模式 $S$ 首次出现的时刻。即：
$$ \tau = \min\{ n \ge L : X_{n-L+1} \dots X_n = S \} $$
其中 $X_k$ 是第 $k$ 次投掷的结果。
根据**鞅停时定理**（又称可选停止定理），在满足一定条件下（本问题满足），鞅过程在停时的期望值等于其初始值：
...
\subsection{模式 1: \texttt{HHHHHHTTTTTT}}
...
\subsection{模式 2: \texttt{HHHHHHHHHHHH}}
...
\subsection{模式 3: \texttt{HTHTHTTHTHT}}
