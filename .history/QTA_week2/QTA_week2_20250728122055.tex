...
$$\int_{-\infty}^{+\infty}{\left(y+\frac{1}{\sqrt{\pi}}\right)^{2}(1-\Phi(y))\varphi
  (y)dy}$$
将积分展开可得到这个积分：
设 $\varphi(y)$ 和 $\Phi(y)$ 分别为标准正态分布的概率密度函数 (PDF) 和累积分布函数 (CDF)。要求解的积分为：
...
我们利用标准正态分布的两个关键性质：
\begin{itemize}
    \item 密度函数 $\varphi(y)$ 是一个偶函数，即 $\varphi(-y) = \varphi(y)$。
    \item 分布函数满足 $\Phi(-y) = 1 - \Phi(y)$。
\end{itemize}

在积分 $I$ 中，令 $z = -y$，则 $y = -z$ 且 $dy = -dz$。积分上下限反转，但可以通过负号抵消：
$$ I = \int_{+\infty}^{-\infty}{\left(-z+\frac{1}{\sqrt{\pi}}\right)^{2}(1-\Phi(-z))\varphi(-z)(-dz)} $$
...
\begin{itemize}
    ...
    \item \textbf{第三部分}: $\frac{2}{\sqrt{\pi}} \left[ \int y\varphi(y)dy - 2\int y\Phi(y)\varphi(y)dy \right]$
        \begin{itemize}
            ...
            \item 对于 $\int y\Phi(y)\varphi(y)dy$，通过分部积分可得其值为 $\frac{1}{2\sqrt{\pi}}$。
