...
\begin{itemize}
    ...
    \item \textbf{第三部分}: $\frac{2}{\sqrt{\pi}} \left[ \int y\varphi(y)dy - 2\int y\Phi(y)\varphi(y)dy \right]$
        \begin{itemize}
            ...
            \item 对于 $\int_{-\infty}^{+\infty} y\Phi(y)\varphi(y)dy$，通过分部积分可得其值为 $\frac{1}{2\sqrt{\pi}}$，具体来说：
            \begin{align*}
                \int_{-\infty}^{+\infty} y\Phi(y)\varphi(y)dy &= \int_{-\infty}^{+\infty} y\Phi(y)\varphi(y)dy \\
