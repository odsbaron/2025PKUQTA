...
\noindent 证明这个公式需要用到双变量正态分布的概率密度函数 (PDF)，并通过一些代数技巧来推导出条件分布的形式。
...
\subsection*{第一步：定义联合概率密度函数 (Joint PDF)}
...
\subsection*{第二步：分解指数项（关键代数步骤）}

这是证明中最核心的技巧：\textbf{对指数项 $Q(x, y)$ 配方法}。我们把所有和 $x$ 相关的项整理成一个平方项。$Q(x, y)$ 中和 $x$ 相关的部分可以看作是关于变量 $(\frac{x-\mu_X}{\sigma_X})$ 的二次方程。我们把它配成一个完全平方：
\begin{align*}
...
\subsection*{第三步：导出条件概率密度函数 (Conditional PDF)}
...
\end{align*}
最后，将 $\mu_X$, $\mu_Y$, $\sigma_Y^2$ 替换为期望和方差的符号：
$$
