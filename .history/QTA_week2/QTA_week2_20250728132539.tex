...
\begin{itemize}
    ...
    \item \textbf{第三部分}: $\frac{2}{\sqrt{\pi}} \left[ \int y\varphi(y)dy - 2\int y\Phi(y)\varphi(y)dy \right]$
        \begin{itemize}
            ...
            \item 对于 $\int_{-\infty}^{+\infty} y\Phi(y)\varphi(y)dy$，通过分部积分可得其值为 $\frac{1}{2\sqrt{\pi}}$
            \begin{align*}
                ...
                \marginnote{\begin{align*}
                    ...
                    \text{Therefore, } \varphi'(y) = -y\varphi(y) \text{ or equivalently } y\varphi(y) = -\varphi'(y)}
...
\begin{lstlisting}[language=Python, caption=O(1) 空间复杂度的解法]
    def rob_optimized(S):
        ...
        for amount in S:
            ...
            temp = curr_max
            # 新的 curr_max (即 dp[i]) 是不偷当前(curr_max)与偷当前(amount + prev_max)的较大值
