...
% 题目背景框
\begin{tcolorbox}[colback=gray!5!white, colframe=black!75!black, title=\textbf{题目背景}]
在金融高频交易中，我们监控一个二元信号流 $S = \{x_1, x_2, \dots\}$，其中 $x_i$ 独立服从伯努利分布 $x_i \sim \text{Bernoulli}(0.5)$（即 $P(H)=P(T)=0.5$）。

我们将特定子序列的出现视为“触发信号”。理解不同信号的平均等待时间（Mean First Passage Time）对于设计低延迟策略至关重要。
\end{tcolorbox}

\vspace{1em}

% Part 1
\section*{Part 1: 基础延迟分析 (Baseline Latency)}
...
\begin{enumerate}[label=\textbf{问题 1.\arabic*:}, leftmargin=3.5em]
    ...
    \item 直观解释为何这两个看似概率相同的模式，其期望等待时间会出现差异？\\
    \textit{（提示：请从“状态重置”或“自相关性”的角度进行阐述。）}
\end{enumerate}
...
% Part 2
\section*{Part 2: 广义模式推导 (Generalization \& Martingales)}
...
\begin{enumerate}[label=\textbf{问题 2.\arabic*:}, leftmargin=3.5em]
    \item 利用鞅（Martingale）的性质，证明对于任意长度为 $L$ 的二进制序列 $A$，其首次出现的期望时间公式为：
    \begin{equation*}
...
\begin{enumerate}[label=\textbf{问题 3.\arabic*:}, leftmargin=3.5em]
    \item 注意，这里不再是计算“期望时间”，而是计算\textbf{获胜概率}。请问 Bob 获胜的概率（即 $B$ 先于 $A$ 出现的概率）是多少？
