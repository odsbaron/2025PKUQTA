...
要理解为何“查找匹配”能够解决问题，我们必须回到描述单次称重物理结果的数学关系式。这个关系式是整个解码策略的基石。
...
\end{itemize}
通过简单的物理分析，我们可以得到以下恒成立的公式：
\begin{equation}
    r = W_j \times s_j
\end{equation}
这个公式精确地将物理操作与数学结果联系了起来。

\section{从单次称重扩展到三次称重}

我们的最优策略包含三次称重。如果第 $j$ 号球是次品，那么上述的核心公式必须对每一次称重都成立。设第 $i$ 次称重的结果为 $r_i$，对球 $j$ 的放置为 $W_{ij}$，则有：
\begin{align*}
