...
\begin{description}
    \item[\textbf{平衡原则 (Balance Principle)}]
    每次称重时，左右两盘的球数必须相等。这意味着矩阵的每一行所有元素之和必须为0。
    $$ \sum_{j=1}^{12} W_{ij} = 0, \quad \text{for } i=1,2,3 $$

    \item[\textbf{唯一性原则 (Uniqueness Principle)}]
    矩阵的每一列向量必须是唯一的。如果两列相同（$\mathbf{W_j} = \mathbf{W_k}$），我们就无法区分 $j$ 号球和 $k$ 号球。

    \item[\textbf{无歧义原则 (No Ambiguity Principle)}]
    ...
    \item[\textbf{有效性原则 (Effectiveness Principle)}]
