...
\subsection{证明}
用决策树刻画比较排序，对 $n$ 个元素共有 $n!$ 种排列，必须对应 $n!$ 个叶子。
二叉决策树高度为 $h$ 时叶子最多 $2^h$ 个，
因此需 $2^h\ge n!$，得 $h\ge\log_2(n!)$。
由斯特林公式 $\ln(n!)\approx n\ln n-n$ 
可知 $\log_2(n!)$ 量级为 $n\log n$，故最坏情况比较次数下界为 $O(n\log n)$。

\textbf{结论：} 最坏时间下界 $O(n\log n)$。
