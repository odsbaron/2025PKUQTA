...
\subsection{解答与推导}
$R$ 可写成 $R = (1-\rho)I + \rho J$，其中 $I$ 为单位阵、$J$ 为全 1 阵。全 1 阵在三维下的特征值是 $3$（重数 1）和 $0$（重数 2），因此
\begin{align*}
    \lambda_1 &= (1-\rho)+3\rho = 1+2\rho,\\
    \lambda_2 &= 1-\rho,\\
    \lambda_3 &= 1-\rho\text{。}
\end{align*}
半正定要求所有特征值非负，于是 $1+2\rho \ge 0$ 且 $1-\rho \ge 0$，得到 $-\tfrac{1}{2} \le \rho \le 1$。
