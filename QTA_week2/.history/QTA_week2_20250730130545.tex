...
如图1所示，当大圆圈不发生移动时，正方形C的四个顶点分别在圆周上移动，形成一个类似圆弧的图形，根据图形我们容易计算出形成的面积是：
\begin{align}
    面积 &= 12\times 12 - 25\times \pi - (100-25\pi) \\
    &= 44
\end{align}
...
\subsubsection*{7.1 应用鞅停时定理建模求解}
对于一个给定的、由 H 和 T 组成的模式 $S = s_1s_2...s_L$，我们的目标是计算首次观察到该模式所需的期望投掷次数，记为 $E$。
...
\subsubsection*{7.2 应用马尔可夫过程建模求解}
